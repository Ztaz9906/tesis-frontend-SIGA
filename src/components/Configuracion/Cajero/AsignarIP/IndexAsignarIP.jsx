...
import { getListWithSearch, } from '../../../../services/services';
import { Field, Form, Formik, useFormik } from 'formik';
import { useDeleteTarjetaMutation, } from '../../../../services/tarjetas.service';
import Swal from 'sweetalert2';
import { useGetAsignarIpQuery } from '../../../../services/asignarip.service';

export const IndexAsignarIP = () => {
    // Estado para la paginación
    const [page, setPage] = useState(0);
    const [rowsPerPage, setRowsPerPage] = useState(5);
    const [showOption, setshowOption] = useState(true)
    const { data, isError, isLoading, isSuccess } = useGetAsignarIpQuery(undefined, {
            ...
            <div className='p-5'>
                <TableContainer component={Paper}>
                    <div className='flex flex-row justify-between px-3 items-center text-center'>
                        <h2 className='text-gray-700 font-semibold text-lg'>Lista de Puertas </h2>
                        <TablePagination
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {data?.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).filter((filter) => {
                                return filter.category.name === 'Puerta';
                            }).map((row) => (
                                <TableRow key={row.id} className='hover:bg-gray-200'>
                                    ...
                                    <TableCell>
                                        <div className='flex flex-row text-center items-center'>
                                            <Link
                                                to={`/configuracion/cajero/asignar-ip/asignar/${row.id}`}
                                                className={`mx-5 `}
