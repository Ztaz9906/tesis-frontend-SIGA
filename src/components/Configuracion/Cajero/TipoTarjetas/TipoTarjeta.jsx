...
export const TipoTarjeta = ({ title }) => {
    ...
    if (id) {
        const fetchData = async () => {
            try {
                const response = await getDetail(id, 'Cajero_TbNtipoTarjeta');
                setData(response.data);

                // Configurar los valores del formulario aquí
                setFormValues({
                    nombre_tipo_tarjeta: response.data ? response.data.nombre_tipo_tarjeta : '',
                    activo: response.data ? response.data.activo : false,
                    descripcion: response.data ? response.data.descripcion : '',
                    id_institucion: 1,
                    color: response.data ? response.data.color : '#ffffff',
                });
            } catch (err) {
    ...
    const formik = useFormik({
        ...
        onSubmit: async values => {
            try {
                if (id) {
                    await update(id, values, 'Cajero_TbNtipoTarjeta');
                    navigate('/configuracion/cajero/tipo_tarjeta')
                }
                else {
                    await create(values, 'Cajero_TbNtipoTarjeta');
                    formik.resetForm();
                }
            } catch (error) {
                ...
                <form onSubmit={formik.handleSubmit}>
                    <div className="mb-4 flex justify-between  items-center">
                        <div className="flex flex-wrap mr-6">
                            ...
                            <input
                                ...
                                value={formik.values.nombre_tipo_tarjeta}
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <input
                                ...
                                className="shadow appearance-none border rounded w-full py-2 px-3 mx-5 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                    ...
                    <div className="flex justify-end">
                        ...
                        <button className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded"
                            type="button"
                            onClick={() => navigate('/configuracion/cajero/tipo_tarjeta')}
                        >
                            Cancelar
                        </button>
                    </div>
