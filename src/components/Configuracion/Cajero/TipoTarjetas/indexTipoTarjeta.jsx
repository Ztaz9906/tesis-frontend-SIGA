...
export const IndexTipoTarjeta = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await getList('Cajero_TbNtipoTarjeta')
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Estás seguro de que quieres borrar esto?')) {
            await remove(id, 'Cajero_TbNtipoTarjeta')
    ...
    return (
        <>
            <Title title="Tipos de Tarjetas" path={'/configuracion/cajero/tipo_tarjeta/create'} />
            <Formik
                initialValues={{
                    active: '',
                }}
            ...
            <div className='p-5'>
                <TableContainer component={Paper}>
                    <div className='flex flex-row justify-between px-3 items-center text-center'>
                        <h2 className='text-gray-700 font-semibold text-lg'>Lista de Tipos de Tarjetas </h2>
                        <TablePagination
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {data.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => (
                                <TableRow key={row.id_tipo_tarjeta} className='hover:bg-gray-200'>
                                    <TableCell>{String(row.nombre_tipo_tarjeta)}</TableCell>
                                    ...
                                    <TableCell>
                                        <div className='flex flex-row text-center items-center'>
                                            ...
                                            <button
                                                onClick={() => handleDelete(row.id_tipo_tarjeta)}><i className="fa-solid fa-trash-can"></i></button>
                                        </div>
                                    </TableCell>
                                </TableRow>
                            ))}
